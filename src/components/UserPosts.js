...
const UserPosts = ({currentUser, displayUser, users, posts, comments, updateCommentHandler, updatePostHandler, freshPostHandler, likePost, deletePost, checkLike, checkOwned, })   => {
    ...
    const filteredUserPosts = filteredPosts.filter(p => p.user === displayUser.id)

    //Pagination related code taken from https://www.youtube.com/watch?v=IYCa1F-OWmk
    const indexOfLastPost = currentPage * postsPerPage
    const indexOfFirstPost = indexOfLastPost - postsPerPage
    const currentPosts = filteredUserPosts.slice(indexOfFirstPost, indexOfLastPost)
    ...
    return(
        <div className="w-100">
            ...
            <Pagination postsPerPage={postsPerPage} totalPosts={filteredUserPosts.length} currentPage={currentPage} paginate={paginate} />
        </div>
    )  
}
